...
# ── Audio file paths ──────────────────────────
GABRIEL_TRAIN_FILE = "./data/audio/gabriel_samples.m4a"
RAIZ_TRAIN_FILE    = "./data/audio/raiz_samples.m4a"
GABRIEL_TEST_FILE  = "./data/audio/gabriel_test.m4a"
RAIZ_TEST_FILE     = "./data/audio/raiz_test.m4a"
...
ACTIVATION_FUNCTIONS = ["sigmoid", "relu", "tanh", "leaky_relu", "elu", "softmax", "swish"]
...
def elu_derivative(a: np.ndarray) -> np.ndarray:
    """Derivative of ELU given its output."""
    return np.where(a >= 0, 1.0, a + ELU_ALPHA)
...
def softmax_derivative(a: np.ndarray) -> np.ndarray:
    """Element-wise approximation of softmax derivative for backprop."""
    # True softmax jacobian is: J_ij = a_i * (δ_ij - a_j)
    # For element-wise approximation in backprop: use a * (1 - a)
    # This is a simplification that works reasonably well as a hidden activation
    return a * (1.0 - a)
...
def swish_derivative(a: np.ndarray, z: np.ndarray = None) -> np.ndarray:
    """Derivative of Swish. Note: requires pre-activation z."""
    # d(swish)/dz = swish(z) + sigmoid(z) * (1 - sigmoid(z)) * z
    # OR: sigmoid(z) * (1 + z * (1 - sigmoid(z)))
    if z is None:
        # Approximate if z not available (less accurate)
        return a + a * (1 - a)
    sig_z = sigmoid(z)
...
# ─────────────────────────────────────────────
# Architecture:
#   Input (16384) → Hidden (5, activation_fn) → Output (1, sigmoid)
#
# Forward pass:
