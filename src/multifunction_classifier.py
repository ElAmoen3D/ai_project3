...
Each run saves its results to a dedicated subfolder under results/multifunction/.
...
ensure_base_dirs()
BASE_OUTPUT_DIR = str(RESULTS_DIR / "multifunction")
...
def run_one(activation: str, X_train: np.ndarray, y_train: np.ndarray,
    ...
    banner = f"  RUN: {label}  →  results/multifunction/{activation}/"
    print(f"\n{'='*55}")
    print(banner)
    print(f"{'='*55}")
