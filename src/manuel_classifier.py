...
Each run saves its results to a dedicated subfolder under results/manual/.
...
ensure_base_dirs()
BASE_OUTPUT_DIR = str(RESULTS_DIR / "manual")
...
def run_one(activation: str, X_fit: np.ndarray, y_fit: np.ndarray,
    ...
    Saves all outputs to results/manual/<activation_name>/.
    ...
    banner = f"  RUN: {label}  →  results/manual/{activation}/"
    print(f"\n{'='*55}")
    print(banner)
    print(f"{'='*55}")
...
def main():
    ...
    for activation in ACTIVATION_FUNCTIONS:
        print(f"    {BASE_OUTPUT_DIR}/{activation}/")
